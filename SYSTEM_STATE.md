...
- **Retry.** A failed `browse_web` run or chart generation used to become a sentence in
  TARS's reply — recovering meant remembering the original request and retyping it in full.
  A new `ToolFailedCard` (rose accent) surfaces the failure with a RETRY button.
- **Retry does not blindly replay the identical call.** It hands the model a plain-language
  "try that again" naming the specific thing that failed, through the same `onAsk` path the
  suggestion chips already use — so the model can act on *why* it failed (a stale browser
  ref wants a fresh attempt, a rate limit wants a pause, a bad chart spec wants different
  code) instead of mechanically repeating whatever just didn't work.
- **Scoped to four tools** — `browse_web`, `archive_page`, `generate_chart`,
  `sync_meetings` — where failure is plausible and the request is nontrivial to retype. Not
  a blanket wrapper: most tool failures already recover in place (an email draft's own Send
  button just works again). `execute_browse_web`/`execute_archive_page` gain an
  `on_failure` callback, the same shape as the existing `on_progress`/`on_artifact`.
- **To-Dos become checkboxes.** `list_reminders` returned a bulleted sentence — readable,
  not actionable, so checking one off meant leaving chat for the To-Dos page. It now also
  emits a `reminders_list` card (`RemindersListCard.tsx`) rendering the same list as real
  checkboxes, PATCHing `/api/reminders/{id}` on click with an optimistic toggle that reverts
  on failure. Overdue-in-rose logic is copied from the To-Dos page itself. The text still
  returned to the model now says the list is already shown as a card, so the model doesn't
  restate it underneath as a redundant bullet list.
- **Verified with real writes, not just rendering.** Locally: seeded an overdue and an
  undated reminder, clicked the checkbox, confirmed `done=true` persisted via a direct query
  — not just an optimistic UI flip — and confirmed the overdue chip computes to the real
  `--c-rose` token (`rgb(160,72,72)`). In production: created a real reminder via the API,
  sent an actual chat message ("List my pending to-dos"), and confirmed a live model turn
  triggered `list_reminders` and rendered the card with the real reminder — the model also
  correctly did not restate it as prose, honoring the new instruction. Test reminder and
  test conversation turns removed afterward; the conversation's title (regenerated by the
  extra turns) was restored to what it was before.
- Web + harness, no schema change.
